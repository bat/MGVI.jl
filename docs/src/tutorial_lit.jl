...
# The output contains an updated parameter estimate (`first_iteration.center`),
...
plot_iteration = (params, label) -> let
    ...
    scatter!(_common_grid, _mean(params.center), markercolor=:green, label=label)
end;
...
plot_iteration_light = (params, counter) -> let
    scatter!(_common_grid, _mean(params.center), markercolor=:green, markersize=3, markeralpha=2*atan(counter/18)/π, label=nothing)
end;
